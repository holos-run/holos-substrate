...
type OIDCConfig struct {
	...
	// An entry whose ValueClaim is absent from a given token is skipped for that
	// request (the extra is optional context, like a missing groups claim); an entry
	// whose claim is present but not a string denies the request fail-closed (a
	// misconfiguration pointing at a list/object/number claim). Each entry's value is
	// a single string in this phase — multi-valued extras are a deferred follow-up.
	//
	// The list is a map keyed by Key (listType=map / listMapKey=key), so the API
	// server rejects duplicate keys at admission. Key must additionally be a valid
	// HTTP header field-name token (the reconciler validates it and rejects the spec,
	// Accepted=False, otherwise) because it is emitted verbatim as the
	// Impersonate-Extra-<key> header suffix. The API server lowercases the extra key
	// it derives from the header name.
	//
	// +optional
	// +listType=map
	// +listMapKey=key
...
type ExtraMapping struct {
	...
	// a valid HTTP header field-name token (RFC 7230); the reconciler rejects a Key
	// with other characters so the authorizer never emits a malformed header. Keys
	// are unique within a Backend (enforced by the listMapKey on Extra). Required.
	//
	// +kubebuilder:validation:MinLength=1
	Key string `json:"key"`

	// ValueClaim is the token claim the extra value is read from. A token missing
	// this claim skips the entry (no header emitted); a present-but-non-string claim
	// denies the request fail-closed. Required.
	//
	// +kubebuilder:validation:MinLength=1
