...
// Cluster defines the API schema for a highly available
...
#ClusterSpec: {
	...
	ephemeralVolumesSizeLimit?: {
		...
		shm?: matchN(>=1, [int, string]) & (number | =~"^(\\+|-)?(([0-9]+(\\.[0-9]*)?)|(\\.[0-9]+))(([KMGTPE]i)|[numkMGTPE]|([eE](\\+|-)?(([0-9]+(\\.[0-9]*)?)|(\\.[0-9]+))))?$")

		// TemporaryData is the size limit of the temporary data volume
		temporaryData?: matchN(>=1, [int, string]) & (number | =~"^(\\+|-)?(([0-9]+(\\.[0-9]*)?)|(\\.[0-9]+))(([KMGTPE]i)|[numkMGTPE]|([eE](\\+|-)?(([0-9]+(\\.[0-9]*)?)|(\\.[0-9]+))))?$")
	}
	...
	// PodSelectorRefs defines named pod label selectors that can be
	// referenced
	// in pg_hba rules using the ${podselector:NAME} syntax in the
	// address field.
	// The operator resolves matching pod IPs and the instance manager
	// expands
	// pg_hba lines accordingly. Only pods in the Cluster's own
	// namespace are considered.
	podSelectorRefs?: [...{
		// Name is the identifier used to reference this selector in
		// pg_hba rules
		...
		// should be resolved. Only pods in the Cluster's namespace are
		// considered.
		selector!: {
			// matchExpressions is a list of label selector requirements. The
			// requirements are ANDed.
	...
	postgresql?: {
		...
		// the rule will be expanded for each Pod IP matching that
		// selector.
		pg_hba?: [...string]
	...
	probes?: {
		...
		readiness?: {
			...
			maximumLag?: matchN(>=1, [int, string]) & (number | =~"^(\\+|-)?(([0-9]+(\\.[0-9]*)?)|(\\.[0-9]+))(([KMGTPE]i)|[numkMGTPE]|([eE](\\+|-)?(([0-9]+(\\.[0-9]*)?)|(\\.[0-9]+))))?$")
	...
	// Name of an existing ServiceAccount in the same namespace to use
	// for the cluster.
	// When specified, the operator will not create a new
	// ServiceAccount
	// but will use the provided one. This is useful for sharing a
	// single
	// ServiceAccount across multiple clusters (e.g., for cloud IAM
	// configurations).
	// If not specified, a ServiceAccount will be created with the
	// cluster name.
