...
// MembershipClient is the seam the KeycloakGroupMembership reconciler drives
...
// MembershipReconciler reconciles one KeycloakGroupMembership CR into membership
// edges on a referenced KeycloakGroup. It owns only the members recorded in
...
func (r *MembershipReconciler) reconcileNormal(ctx context.Context, logger logr.Logger, membership *keycloakv1alpha1.KeycloakGroupMembership) (ctrl.Result, error) {
	...
	if !sameInstanceRef(membership.Namespace, membership.Spec.InstanceRef, group.Namespace, group.Spec.InstanceRef) {
		message := fmt.Sprintf("membership instanceRef does not match referenced KeycloakGroup %s/%s instanceRef", group.Namespace, group.Name)
...
func (r *MembershipReconciler) reconcileDelete(ctx context.Context, logger logr.Logger, membership *keycloakv1alpha1.KeycloakGroupMembership) (ctrl.Result, error) {
	...
	r.Recorder.Event(membership, corev1.EventTypeNormal, ReasonReleased,
		fmt.Sprintf("released KeycloakGroupMembership %s/%s", membership.Namespace, membership.Name))
	logger.Info("released KeycloakGroupMembership", "namespace", membership.Namespace, "name", membership.Name)
	return r.removeFinalizer(ctx, membership)
}

func (r *MembershipReconciler) resolveInstance(ctx context.Context, membership *keycloakv1alpha1.KeycloakGroupMembership) (*keycloakv1alpha1.KeycloakInstance, ctrl.Result, error) {
	...
	if instanceNamespace != membership.Namespace {
		allowed, err := referencegrant.Allowed(ctx, r.Client,
			referencegrant.FromRef{Group: keycloakv1alpha1.GroupVersion.Group, Kind: "KeycloakGroupMembership", Namespace: membership.Namespace},
			referencegrant.ToRef{Group: keycloakv1alpha1.GroupVersion.Group, Kind: "KeycloakInstance", Namespace: instanceNamespace, Name: ref.Name},
		)
		if err != nil {
...
func (r *MembershipReconciler) resolveGroup(ctx context.Context, membership *keycloakv1alpha1.KeycloakGroupMembership) (*keycloakv1alpha1.KeycloakGroup, ctrl.Result, error) {
	...
	if groupNS != membership.Namespace {
		allowed, err := referencegrant.Allowed(ctx, r.Client,
			referencegrant.FromRef{Group: keycloakv1alpha1.GroupVersion.Group, Kind: "KeycloakGroupMembership", Namespace: membership.Namespace},
			referencegrant.ToRef{Group: keycloakv1alpha1.GroupVersion.Group, Kind: "KeycloakGroup", Namespace: groupNS, Name: membership.Spec.GroupRef.Name},
		)
		if err != nil {
			return nil, ctrl.Result{}, fmt.Errorf("checking ReferenceGrant for KeycloakGroup %s/%s: %w", groupNS, membership.Spec.GroupRef.Name, err)
		}
		if !allowed {
			message := fmt.Sprintf("cross-namespace reference to KeycloakGroup %s/%s is not authorized by a security.holos.run ReferenceGrant", groupNS, membership.Spec.GroupRef.Name)
			result, rerr := r.notReady(ctx, membership, ReasonReferenceNotGranted, message)
			return nil, result, rerr
		}
	}
	...
	if err := r.Get(ctx, key, group); err != nil {
		if apierrors.IsNotFound(err) {
			message := fmt.Sprintf("referenced KeycloakGroup %s/%s does not exist", groupNS, membership.Spec.GroupRef.Name)
			result, rerr := r.notReady(ctx, membership, ReasonGroupNotReady, message)
			return nil, result, rerr
		}
		return nil, ctrl.Result{}, fmt.Errorf("resolving KeycloakGroup %s/%s: %w", groupNS, membership.Spec.GroupRef.Name, err)
	}
	if !groupReady(group) {
		message := fmt.Sprintf("referenced KeycloakGroup %s/%s is not Ready", groupNS, membership.Spec.GroupRef.Name)
		result, rerr := r.notReady(ctx, membership, ReasonGroupNotReady, message)
		return nil, result, rerr
	}
...
func managedMembersByEmail(entries []keycloakv1alpha1.ManagedKeycloakGroupMember) map[string]string {
...
func serializeManagedMembers(managed map[string]string) []keycloakv1alpha1.ManagedKeycloakGroupMember {
	if len(managed) == 0 {
		return nil
	}
	...
	sort.Strings(emails)
	out := make([]keycloakv1alpha1.ManagedKeycloakGroupMember, 0, len(emails))
	for _, email := range emails {
		out = append(out, keycloakv1alpha1.ManagedKeycloakGroupMember{Email: email, UserID: managed[email]})
	}
	return out
}
...
func (r *MembershipReconciler) peerStillDesires(ctx context.Context, membership *keycloakv1alpha1.KeycloakGroupMembership, email string) bool {
	var list keycloakv1alpha1.KeycloakGroupMembershipList
	if err := r.List(ctx, &list); err != nil {
		log.FromContext(ctx).Error(err, "listing peer KeycloakGroupMemberships before prune")
		return true
	}
...
func sameMembershipTarget(a, b *keycloakv1alpha1.KeycloakGroupMembership) bool {
...
func defaultedGroupRef(namespace string, ref keycloakv1alpha1.KeycloakGroupReference) string {
	ns := ref.Namespace
	if ns == "" {
		ns = namespace
	}
...
func (r *MembershipReconciler) succeed(ctx context.Context, logger logr.Logger, membership *keycloakv1alpha1.KeycloakGroupMembership, reason, message string, extraChanged bool) (ctrl.Result, error) {
	...
	logger.Info("reconciled KeycloakGroupMembership", "namespace", membership.Namespace, "name", membership.Name, "reason", reason)
...
func (r *MembershipReconciler) notReady(ctx context.Context, membership *keycloakv1alpha1.KeycloakGroupMembership, reason, message string) (ctrl.Result, error) {
...
func (r *MembershipReconciler) reject(ctx context.Context, membership *keycloakv1alpha1.KeycloakGroupMembership, reason, message string) (ctrl.Result, error) {
...
func (r *MembershipReconciler) memberNotFound(ctx context.Context, membership *keycloakv1alpha1.KeycloakGroupMembership, message string, extraChanged bool) (ctrl.Result, error) {
...
func (r *MembershipReconciler) handleCredentialError(ctx context.Context, membership *keycloakv1alpha1.KeycloakGroupMembership, err error) (ctrl.Result, error) {
...
func (r *MembershipReconciler) removeFinalizer(ctx context.Context, membership *keycloakv1alpha1.KeycloakGroupMembership) (ctrl.Result, error) {
...
func (r *MembershipReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&keycloakv1alpha1.KeycloakGroupMembership{}, builder.WithPredicates(predicate.GenerationChangedPredicate{})).
		Watches(&keycloakv1alpha1.KeycloakInstance{}, handler.EnqueueRequestsFromMapFunc(r.membershipsForInstance)).
		Watches(&keycloakv1alpha1.KeycloakGroup{}, handler.EnqueueRequestsFromMapFunc(r.membershipsForGroup)).
		Complete(r)
}
...
func (r *MembershipReconciler) membershipsForInstance(ctx context.Context, obj client.Object) []ctrlreconcile.Request {
	...
	var memberships keycloakv1alpha1.KeycloakGroupMembershipList
