...
type Identity struct {
	...
	// (spec.oidc.usernameClaim, default "sub").
	Username string
...
// mapper, and the username claim. usernameClaim defaults to "sub" when empty,
// matching the OIDC convention and the Backend CRD default.
func NewAuthenticator(verifier TokenVerifier, mapper *GroupMapper, usernameClaim string) *Authenticator {
	if usernameClaim == "" {
		usernameClaim = "sub"
	}
...
// configured username claim and maps the claims to groups via the compiled CEL
// expression.
//
