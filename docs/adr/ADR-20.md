...
- **Previously-refused specs now enter the normal claim/adoption flow.** Any
  ...
  transparent controller, **no longer rejected** — it is reconciled through the
  same claim/adoption model every other CR uses. The retained ownership model
  still governs the outcome: when the controller creates or already owns the
  target object it **writes the spec verbatim**; when an **unadopted, foreign**
  object already holds that name the CR surfaces **`Conflict`** (not a write, and
  not a seizure) unless `spec.adopt` is set. The net change is that the name is no
  longer **refused outright** on policy grounds — so a write that was previously a
  no-op rejection can now reach Keycloak. Operators with such objects should
