...
- [ADR-22 — The `security.holos.run` API Group and `ReferenceGrant`](ADR-22.md):
  ...
  project namespace references an `Instance` in a platform namespace; that
...
- **`api/keycloak/v1alpha1` imports only `k8s.io/api` and `k8s.io/apimachinery`**
  ...
  named by an `Instance`'s `credentialsSecretRef`. The API package stays
...
Because an `Instance` is a plain namespaced CR carrying its own `apiURL` +
...
### Every Kind references an `Instance`, gated by a `ReferenceGrant` (AC #4.4, #4.5)
...
- a **per-Kind reconcile counter** labeled by `kind` (`instance` /
  `client` / `group` / `membership` / `user`) and `outcome`
  (`success` / `error`); and
...
3. **Every other Kind references an `Instance` via `instanceRef`**, and a
