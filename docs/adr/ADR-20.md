...
spec:
  instanceRef:                     # immutable; cross-namespace gated by ReferenceGrant
    name: holos-keycloak
    namespace: keycloak
  groupRef:                        # immutable; the KeycloakGroup CR being bound
    name: my-project-roles-owner   # same namespace by default
...
- **`spec.groupRef`** is immutable and names the `KeycloakGroup` whose Keycloak
  group membership is being managed. A same-namespace `groupRef` is implicitly
  authorized. A cross-namespace `groupRef` requires a `security.holos.run`
  `ReferenceGrant` in the **group's namespace** (the referent namespace) whose
  `from` entry is `group: keycloak.holos.run`, `kind:
  KeycloakGroupMembership`, and the referrer's namespace, and whose `to` entry is
  `group: keycloak.holos.run`, `kind: KeycloakGroup`, optionally narrowed by
  `name`. No matching grant means fail-closed: the reconciler sets `Ready=False`
  with reason `ReferenceNotGranted` and does not mutate Keycloak.
...
- **Keycloak-console delegation** remains FGAP v2 through
  ...
  `roles/<role>` group directly in the console. The Project component seeds
  project owners into `projects/<name>/custodians/{owner,editor,viewer}` so the
  console-side delegation path is live for the generated project scaffold.
- **Kubernetes-API delegation** is `KeycloakGroupMembership` in the target
  ...
  personas RBAC to create/update membership CRs there, using the aggregated
  ClusterRole labels the API implementation adds. This realizes ADR-3's
  custodian-approved group membership on the Kubernetes API plane while keeping
  the double-binding guard: the writer needs RBAC in the group's namespace, and a
  cross-namespace target still needs a `ReferenceGrant` from the group owner.
