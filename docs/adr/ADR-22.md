...
| 5        | 2026-07-07 | @jeffmccune | **Implement Adopt & Preserve across the shipped external-resource APIs (HOL-1534..HOL-1537).** Quay Organization/Repository and Keycloak Group/User/Client/GroupMembership now carry the lifecycle fields and finalizer behavior described in Rev 4, with KeycloakInstance kept as the read-only validator exemption and KeycloakGroupMembership kept as the set-membership manager exemption from `spec.adopt`. The controller test suites cover the rename/transfer sequence for Quay Organization, Quay Repository, and KeycloakGroup; the operator runbook documents the per-kind transfer procedures, reference cascades, and marker/UUID verification. |
