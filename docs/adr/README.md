...
| [ADR-19](ADR-19.md) | api, controller, quay, registry | **Proposed** — The `quay.holos.run` Organization and Repository CRDs reconciled by the Holos Controller (ADR-18): org name + OIDC group→team/role access + repo-creation toggle; repo retention + a `repo_push` webhook to a Kargo Warehouse; reconciled via the Quay REST API with the ADR-15 superuser OAuth-Application token; replaces the manual org/repo/pull-robot/Argo CD-Secret/webhook provisioning in the credentials runbook and the `my-project` scaffold; updates ADR-15 |
...
| [ADR-21](ADR-21.md) | holos, components, projects, gitops, multi-tenancy | **Proposed** — The Holos Project and Application CUE components delivering one-line self-service: a `holos/projects/*.cue` entry renders 8 project-level resources (a central-registry Namespace entry, the Kargo Project — which brings the shared ProjectConfig + receiver-token bootstrap — the Argo CD AppProject/Application in `argocd`, owner RoleBinding, Quay Organization (ADR-19), ReferenceGrant, HTTPRoute) and a `holos/apps/*.cue` entry renders 11 application-level resources (Quay Repository, Kargo Warehouse/Stage + blue-green pipeline, Argo CD Application in `argocd`, and Deployment/Service/ExternalSecret/ConfigMap/ServiceAccount/RoleBinding workloads in the project namespace); apps are unified under projects by GCP-model containment (Project ≈ Namespace security boundary, `apps.<name>.project` binding); generalizes the `my-project` scaffold; integrates the central `holos/namespaces.cue` registry; updates ADR-1 (maps the Project tenant onto Kubernetes under GitOps); design record only |
