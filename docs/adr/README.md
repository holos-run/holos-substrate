...
| [ADR-18](ADR-18.md) | controller, api, gitops | The Holos Controller (namespace `holos-controller`) reconciles CRDs filling the Quay/Keycloak data-plane gaps the upstream operators leave open (first group `quay.holos.run`); delivery is the GitOps rendered-manifest pattern; refines ADR-12's API-group example and supersedes the ADR-15 (Rev 4–5) manual Quay Resource Controller stop-gap |
