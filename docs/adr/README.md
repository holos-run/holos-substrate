...
| [ADR-21](ADR-21.md) | holos, components, projects, gitops, multi-tenancy | **Proposed** — The Holos Project and Application CUE components delivering one-line self-service: a `holos/projects/*.cue` entry renders 8 project-level resources (registry-integrated Namespace, Kargo Project, Argo CD AppProject/Application, owner RoleBinding, Quay Organization (ADR-19), ReferenceGrant, HTTPRoute) and a `holos/apps/*.cue` entry renders 11 application-level resources (Quay Repository, Kargo Warehouse/Stage + blue-green pipeline, Argo CD Application, Deployment/Service/ExternalSecret/ConfigMap/ServiceAccount/RoleBinding) into the project's namespace; apps are unified under projects by GCP-model containment (Project ≈ Namespace security boundary, `apps.<name>.project` binding); generalizes the `my-project` scaffold; integrates the central `holos/namespaces.cue` registry; updates ADR-1 (maps the Project tenant onto Kubernetes under GitOps); design record only |
