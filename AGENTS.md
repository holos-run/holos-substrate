...
- **Related:** `holos/components/keycloak/realm-config/buildplan.cue` (the `quay` client — **no** `pkce.code.challenge.method` attribute (HOL-1317); the `svc-quay-resource-controller`/`quay-admin` realm users), `holos/components/quay/buildplan.cue` (`AUTHENTICATION_TYPE: OIDC`, `FEATURE_TEAM_SYNCING: true`, `FEATURE_SUPERUSERS_FULL_ACCESS: true`, `USE_PKCE: false` (HOL-1317), the `SUPER_USERS` list, the `KEYCLOAK_LOGIN_CONFIG` block), `docs/adr/ADR-15.md` (Revision 6), `docs/adr/ADR-18.md` (the shipped Holos Controller — the Quay Resource Controller, `Partially Implemented`), `docs/adr/ADR-19.md` (the `quay.holos.run` Organization/Repository CRDs, `Implemented`), `holos/docs/keycloak-clients.md` (the PKCE guardrail checklist), `docs/runbooks/quay-keycloak-oidc.md` (the operational runbook), `docs/runbooks/quay-resource-controller-credentials.md` (the manual superuser OAuth-Application credential procedure, including the `platform-automation` org bootstrap and the full-access semantics), and `docs/runbooks/holos-controller.md` (wiring the controller to that credential, AC #3).
