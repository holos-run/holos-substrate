...
- **Reference:** `holos/components/my-project/buildplan.cue` (the emitted Organization CR + `spec.syncedTeams[]` + gated `spec.caBundle` + the `keycloak.holos.run` Group/User/Client CRs, HOL-1348), `holos/components/keycloak/keycloak-instance/buildplan.cue` (the central `KeycloakInstance` + `security.holos.run` `ReferenceGrant`), `holos/components/keycloak/realm-config/buildplan.cue` (the `svc-holos-controller` admin-credential client + bootstrap + first-broker-login auto-link flow), `scripts/apply-my-project` (the dedicated apply step that injects the local-ca PEM and applies the `keycloak-instance` + project Keycloak CRs), `docs/runbooks/holos-controller.md` (the Keycloak admin credential wiring), `holos/README.md` (*The `my-project` delivery scaffold*), `holos/docs/oci-publish-workflow.md` (*Downstream: the `my-project` delivery scaffold*), `docs/adr/ADR-16.md`, `docs/adr/ADR-19.md` (the `caBundle` convention and the synced-teams design, Revision 6), `docs/adr/ADR-20.md` (the `keycloak.holos.run` group, `Partially Implemented`).
...
- [docs/adr/](docs/adr/README.md) — Architecture Decision Records: the
  ...
  API group CRDs, **`Partially Implemented`** as built in HOL-1344..HOL-1348,
  ...
  reconciles `KeycloakInstance`/`Group`/`User`/`Client` and the `my-project`
  scaffold emits the project CRs; only the generalized Project/Application
  self-service experience (ADR-21) remains proposed.
