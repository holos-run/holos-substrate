...
- **How to apply:** Write the comment as if it were the only context available to a user inspecting the CRD. Describe what the field does, when it is optional, what happens when it is omitted, and any externally visible ownership or lifecycle semantics. Keep implementation notes in controller/internal package comments instead. Run `make controller-generate controller-manifests` after API comment or marker changes, then review the generated Quay CRDs for clear descriptions.
- **Reference:** `api/quay/v1alpha1/organization_types.go`, `api/quay/v1alpha1/repository_types.go`, `config/crd/bases/quay.holos.run_organizations.yaml`, and `config/crd/bases/quay.holos.run_repositories.yaml` are the worked example.

### Forward-compatible unreleased holos.run APIs
- **Rule:** Unreleased `holos.run` CRDs MUST preserve room for future compatible evolution: use pointers for optional structs and for booleans whose `false` state is meaningful (`*bool`); centralize enum constants on the owning API type rather than redefining per field; put validation bounds (`MaxLength`, `MaxItems`, patterns, minimums) on every user-supplied string and list; mark lists with explicit `+listType` semantics; define condition type and reason constants in the API package and re-export them from controllers only for compatibility; and never change JSON field names once CUE templates emit them.
- **Why:** These APIs are still pre-release, so this is the time to get shapes right. Once CUE templates, rendered manifests, and users depend on JSON names and list semantics, changes become migrations instead of refactors. Pointer optionality and centralized enums avoid ambiguous zero values and duplicated vocabularies.
- **How to apply:** Before adding or revising a field, decide whether absence is distinct from an explicit zero value; if it is, use a pointer. Add length/count/pattern validation for all tenant-supplied strings and slices, choose map/set/atomic list semantics intentionally, and place shared condition/reason constants in `api/<group>/<version>`. For generated consumers, treat existing JSON names as stable even if Go field names are improved.
