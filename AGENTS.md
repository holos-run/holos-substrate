...
- **Data plane deferred (design of record now exists):** in-cluster Quay org/repo/robot/webhook provisioning is **deferred to a future Quay Resource Controller** (HOL-1293), now **designed as the Holos Controller** ([ADR-18](docs/adr/ADR-18.md)) with the `quay.holos.run` Organization/Repository CRDs ([ADR-19](docs/adr/ADR-19.md), `Updates: ADR-15`). Until that controller ships, an operator mints the controller's OAuth-Application credential by hand (see the credentials runbook below) — the manual procedure stays operationally true in the interim. The removed `scripts/quay-init`/`scripts/quay-reset` helpers and the `my-project-quay-bootstrap` Job no longer exist.
...
- [docs/adr/](docs/adr/README.md) — Architecture Decision Records: the
  ...
  components, `Updates: ADR-1`) — these are the design of record for the
  formerly-deferred "future Quay Resource Controller" and the Project/Application
