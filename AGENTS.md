...
- **Rule:** Every `holos.run` custom resource MUST report rich status following the Quay/Gateway-API model: a `status.conditions[]` slice of standard `metav1.Condition` (`+listType=map`, `+listMapKey=type`, merge-patch on `type`) with the standard `Accepted`/`Programmed`/`Ready` condition types, plus a `status.observedGeneration` recording the last `spec` generation reconciled, plus at least one printer column surfacing `Ready` (`+kubebuilder:printcolumn:name="Ready",type=string,JSONPath=`.status.conditions[?(@.type=="Ready")].status``). Define the condition types and reasons once (shared constants) rather than re-deriving per reconciler.
