...
- **Rule:** Every new `holos.run` CR whose reconciler fronts an external system (Keycloak, Quay, or a future remote API) MUST report drift-observability status in addition to conditions, and existing shipped external-resource CRs are retrofitted by HOL-1459: `status.lastValidatedTime` (`metav1.Time`, optional), `status.lastMutatedTime` (`metav1.Time`, optional), `status.lastMutationReason` (optional enum with canonical values `SpecChange` / `DriftRemediation`), and `status.lastDriftTime` (`metav1.Time`, optional). Read-only validators that never mutate the remote system, such as `KeycloakInstance`, carry `lastValidatedTime` only and omit the mutation fields. CRs with no external surface are out of scope.
