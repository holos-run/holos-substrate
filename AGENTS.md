...
- **Why manual:** the OIDC backend makes the Keycloak realm the sole identity store, which is the deliberate trade for declarative identity (no second password store, no break-glass local admin). Quay ships no operator to mint a first superuser token declaratively, so the bootstrap is a documented manual step until the future **Quay Resource Controller** — designed as the **Holos Controller** ([ADR-18](docs/adr/ADR-18.md)) with the `quay.holos.run` CRDs ([ADR-19](docs/adr/ADR-19.md)) — ships and takes over the **org/repo/robot/webhook provisioning**. Per ADR-18 the controller still *consumes* this superuser OAuth-Application token (it authenticates to Quay with the credential the runbook mints, read from the `quay-resource-controller` Secret) — the token is the controller's external credential, not one of the CRDs it reconciles. The manual step stays operationally true in the interim, until the controller ships.
