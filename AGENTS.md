...
- **Reference:** [ADR-22](docs/adr/ADR-22.md) (Rev 4 — Adopt & Preserve lifecycle contract), [ADR-19](docs/adr/ADR-19.md) (Quay Organization/Repository intended worked examples: `deletionPolicy`, kind-prefixed Organization adopted marker, and per-team adopt), [ADR-20](docs/adr/ADR-20.md) (Keycloak intended worked examples: `deletionPolicy` across Group/User/Client/GroupMembership, with `KeycloakInstance` as the read-only validator exemption), `api/quay/v1alpha1/organization_types.go`, `api/quay/v1alpha1/repository_types.go`, `api/keycloak/v1alpha1/group_types.go`, `api/keycloak/v1alpha1/user_types.go`, and `api/keycloak/v1alpha1/client_types.go`.
