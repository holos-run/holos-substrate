...
- **Rule:** Every new `holos.run` CR whose reconciler fronts an external system (Keycloak, Quay, or a future remote API) MUST report drift-observability status in addition to conditions; HOL-1454 records the guardrail, and HOL-1459 retrofits existing shipped external-resource CRs: `status.lastValidatedTime` (`metav1.Time`, optional), `status.lastMutatedTime` (`metav1.Time`, optional), `status.lastMutationReason` (optional enum with canonical values `SpecChange` / `DriftRemediation`), and `status.lastDriftTime` (`metav1.Time`, optional). Read-only validators that never mutate the remote system, such as `Instance`, carry `lastValidatedTime` only and omit the mutation fields. CRs with no external surface are out of scope.
...
### keycloak.holos.run Kind names match upstream Keycloak resource types
- **Rule:** Every resource-owning Kind registered in the `keycloak.holos.run` scheme MUST map from CamelCase to an `org.keycloak.events.admin.ResourceType` value in SCREAMING_SNAKE form (`GroupMembership` to `GROUP_MEMBERSHIP`). A Kind that fronts no upstream resource, such as the connection-only `Instance`, MUST have an explicit, justified exception in the scheme-driven guard test.
- **Why:** Kubernetes resources and Keycloak admin events need one shared resource vocabulary. Matching the upstream names prevents local prefixes or near-synonyms from obscuring which Keycloak object a CR manages and keeps event integration mechanically predictable.
- **How to apply:** Before registering a new Kind, check the `ResourceType` enum for the repository's pinned Keycloak version and choose the corresponding CamelCase Kind name. Add an exception only for a connection or validator Kind that owns no upstream Keycloak resource, and include the reason in `kindMappingExceptions`; do not bypass the test with a hardcoded Kind list.
- **Reference:** `api/keycloak/v1alpha1/kind_mapping_test.go` enforces the mapping for every registered non-List Kind; `docs/adr/ADR-20.md` defines the Keycloak API-group boundary and naming model.
...
- **Reference:** `holos/components/project/buildplan.cue` (the Project component — Argo CD/Kargo/Quay Organization + `syncedTeams[]` + gated `caBundle` + the `keycloak.holos.run` Group/User/Client CRs + the three-way role→client bindings), `holos/components/application/buildplan.cue` (the Application component — workload + control-plane bundles, the app `Client` roles), `holos/projects/my-project.cue` + `holos/apps/my-app.cue` (the reference registrations), `holos/projects/projects.cue` + `holos/apps/apps.cue` (`#Project`/`#App` schemas), `holos/collections.cue` (the ancestor/import wiring), `holos/namespaces.cue` (the env-prefixed derivation), `holos/components/keycloak/keycloak-instance/buildplan.cue` (the central `Instance` + `security.holos.run` `ReferenceGrant`), `scripts/apply-projects` (the dedicated apply step injecting the local-ca PEM), `holos/docs/project-and-application-templates.md` (the authoring guide), `holos/README.md` (*The `my-project` delivery scaffold*), `holos/docs/oci-publish-workflow.md`, `docs/adr/archive/ADR-21.md` (`Implemented`, Rev 4), `docs/adr/ADR-19.md`, `docs/adr/ADR-20.md`.
...
- [docs/adr/](docs/adr/README.md) — Architecture Decision Records: the
  ...
  `GroupMembership` design and Rev 10 removes the former
  user-side membership field from `User`),
  ...
  **mint/cache/rotate** the *outbound* impersonator token via TokenRequest
  ...
  reconciles `Instance`/`Group`/`User`/`Client`, and the **collection-driven
