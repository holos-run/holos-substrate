...
- **Why manual:** the OIDC backend makes the Keycloak realm the sole identity store, which is the deliberate trade for declarative identity (no second password store, no break-glass local admin). Quay ships no operator to mint a first superuser token declaratively, so the bootstrap is a documented manual step. The future **Quay Resource Controller** — designed as the **Holos Controller** ([ADR-18](docs/adr/ADR-18.md)) with the `quay.holos.run` CRDs ([ADR-19](docs/adr/ADR-19.md)) — automates the **org/repo/robot/webhook provisioning** that this superuser token enables, **not** the minting of the token itself: ADR-19 has the controller *read* the manually-minted `quay-resource-controller` Secret as its input. So even once the controller ships, this token remains a manual bootstrap (its own automation is left to the controller's implementation phase); the manual step stays operationally true in the interim.
