...
- **Pattern:** A project that receives Kargo-driven OCI delivery is laid down as a **single component** that emits, together, a hand-authored Argo CD AppProject + OCI-source Application (with `kargo.akuity.io/authorized-stage` and `targetRevision` omitted so Kargo owns it) and the Kargo Project/ProjectConfig/Warehouse/Stage. The matching Quay data plane (org/repo/pull-robot/`repo_push` webhook) is **not** emitted by the current hand-authored `my-project` component — its provisioning is deferred to a future Quay Resource Controller, for which the proposed (not-yet-implemented) design is the Holos Controller ([ADR-18](docs/adr/ADR-18.md)/[ADR-19](docs/adr/ADR-19.md)). Under that design the proposed Holos Project/Application components ([ADR-21](docs/adr/ADR-21.md)) — generalizing this `my-project` scaffold — would emit the `quay.holos.run` Organization/Repository CRs per project/app, and the controller reconciles them into the actual Quay data plane (see the next bullet); the removed Database-backend `my-project-quay-bootstrap` Job no longer participates. The project Namespace is **not** emitted by the component either — it is registered centrally in `holos/namespaces.cue` (never inline, per the component guidelines) and referenced by name. The Kargo Project namespace doubles as the workload namespace (no separate `kargo-project-*` sibling — that split is only the echo spike). `my-project` is the reference instance and the template for a future self-service `ProjectRequest`.
- **Quay org/repo/webhook provisioning is deferred (HOL-1293); proposed design in ADR-18/ADR-19/ADR-21:** the in-cluster Quay data plane a project needs — its org, repos, push robot, the Argo CD pull-credential repository Secret in `argocd`, and the `repo_push` webhook pointing at the Kargo receiver URL (read from `ProjectConfig.status`) — is **deferred to a future Quay Resource Controller**. The proposed (not-yet-implemented) design is the Holos Controller ([ADR-18](docs/adr/ADR-18.md)) reconciling the `quay.holos.run` Organization/Repository CRDs ([ADR-19](docs/adr/ADR-19.md)) that the proposed Holos Project/Application components ([ADR-21](docs/adr/ADR-21.md)) would emit per project/app — distinct from today's single hand-authored `my-project` component, which emits the Argo CD/Kargo objects but **not** that Quay data plane. The Database-backend bootstrap Job (`my-project-quay-bootstrap`), the `quay-initial-admin` superuser token it authenticated with, and the `scripts/quay-init`/`scripts/quay-reset` helpers were **removed** with the OIDC switch and no longer exist. Until the controller ships, an operator mints the controller's OAuth-Application credential and provisions the project scaffolding by hand (see `docs/runbooks/quay-resource-controller-credentials.md`); the project's Argo CD Application stays `Unknown`/`Missing` until the first config artifact is published — expected scaffolding.
...
- **Why manual:** the OIDC backend makes the Keycloak realm the sole identity store, which is the deliberate trade for declarative identity (no second password store, no break-glass local admin). Quay ships no operator to mint a first superuser token declaratively, so the bootstrap is a documented manual step until the future **Quay Resource Controller** — designed as the **Holos Controller** ([ADR-18](docs/adr/ADR-18.md)) with the `quay.holos.run` CRDs ([ADR-19](docs/adr/ADR-19.md)) — ships and takes over the **org/repo/robot/webhook provisioning**. Per ADR-18 the controller still *consumes* this superuser OAuth-Application token (it authenticates to Quay with the credential the runbook mints) — the token is the controller's external credential, not one of the CRDs it reconciles; ADR-19 and the runbook specify the exact contract (the `quay-resource-controller` Secret, key `token`, in the `quay` namespace). The manual step stays operationally true in the interim, until the controller ships.
...
- **How to apply:** Follow `docs/runbooks/quay-resource-controller-credentials.md` to create the OAuth Application, choose its scopes (e.g. `super:user`/`org:admin`/`repo:create`), generate the token, and store it as a Kubernetes Secret for the future controller. Store the token as a Secret's *material* per the *Runtime Secret Handling* guardrail — never commit it.
- **Reference:** `holos/components/quay/buildplan.cue` (`AUTHENTICATION_TYPE: OIDC`, `SUPER_USERS`, `FEATURE_SUPERUSERS_FULL_ACCESS: true`), `holos/components/keycloak/realm-config/buildplan.cue` (the `svc-quay-resource-controller`/`quay-admin` realm users + password Secrets), `docs/runbooks/quay-resource-controller-credentials.md` (the manual credential procedure, the `platform-automation` org bootstrap, and the full-access semantics), `docs/runbooks/quay-keycloak-oidc.md` (the OIDC model and superuser verification), `docs/adr/ADR-15.md` (Revision 6), `docs/adr/ADR-18.md`/`docs/adr/ADR-19.md` (the Holos Controller + Quay CRDs that will automate the org/repo/robot/webhook provisioning; the controller still consumes this superuser token as its external credential).
...
- [docs/adr/](docs/adr/README.md) — Architecture Decision Records: the
  ...
  its GitOps rendered-manifest delivery model), ADR-19 (`quay.holos.run`
  Organization/Repository CRDs, `Updates: ADR-15`), ADR-20 (the Keycloak API
  group CRDs, `Updates: ADR-3`), and ADR-21 (the Holos Project/Application
  components, `Updates: ADR-1`) — these are the **proposed** design (all four are
  `Status: Proposed`) for the formerly-deferred "future Quay Resource Controller"
  and the Project/Application self-service experience, not yet implemented.
...
- [docs/runbooks/quay-resource-controller-credentials.md](docs/runbooks/quay-resource-controller-credentials.md)
  ...
  headless `quay-initial-admin` bootstrap. This manual procedure is the
  **interim** step until the Holos Controller ([ADR-18](docs/adr/ADR-18.md)) and
  its `quay.holos.run` CRDs ([ADR-19](docs/adr/ADR-19.md)) automate the
